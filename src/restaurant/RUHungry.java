...
public class RUHungry {
    ...
    public void menu(String inputFile) {
        ...
        int lengthVar = StdIn.readInt();
        StdIn.readLine();
        categoryVar = new String[lengthVar];
        menuVar = new MenuNode[lengthVar];
        for (int i = 0; i < lengthVar; i++) {
            ...
            for (int j = 0; j < dishCount; j++) {
                ...
                int numOfIngredients = Integer.parseInt(info[0]);
                int[] info2 = new int[numOfIngredients];
                for (int k = 1; k <= numOfIngredients; k++) {
                    info2[k-1] = Integer.parseInt(info[k]);
                }
                Dish dish = new Dish(categoryVar[i], dishName, info2);
    ...
    public void addStockNode ( StockNode newNode ) {
        Ingredient s = newNode.getIngredient();
        int index = s.getID() % stockVarSize;
    ...
    public void updateStock (String ingredientName, int ingredientID, int stockAmountToAdd) {
        if(ingredientName == null){
            StockNode target = findStockNode(ingredientID);
            int toAdd = target.getIngredient().getStockLevel();
            target.getIngredient().setStockLevel(stockAmountToAdd + toAdd);
        }
        if(ingredientID == -1){
            StockNode target = findStockNode(ingredientName);
            int toAdd = target.getIngredient().getStockLevel();
            target.getIngredient().setStockLevel(stockAmountToAdd + toAdd);
        }
	// WRITE YOUR CODE HERE
    ...
    public void updatePriceAndProfit() {
        for (int i = 0; i < menuVar.length; i++) {
            MenuNode ptr2 = menuVar[i];
            while(ptr2 != null){
                Dish theDish = ptr2.getDish();
                int[] index = theDish.getStockID();
                double total = 0.0;
                for (int j = 0; j < index.length; j++) {
                    for (int k = 0; k < stockVarSize; k++) {
                        StockNode ptr = stockVar[k];
                        while(ptr!=null){
                            if(index[j] == ptr.getIngredient().getID()){
                                total = total + ptr.getIngredient().getCost();
                            }
                            ptr = ptr.getNextStockNode();
                    }
                    }
                }
                double priceTotal = total * 1.2;
                ptr2.getDish().setPriceOfDish(priceTotal);
                ptr2.getDish().setProfit(priceTotal - total);
    ...
    public void createStockHashTable (String inputFile){
        ...
        while(StdIn.readLine() != null) {
            ...
            int stocklvl = StdIn.readInt();
            Ingredient b = new Ingredient(id, str, stocklvl, price);
            StockNode newNode = new StockNode(b, null);
    ...
    public boolean checkDishAvailability (String dishName, int numberOfDishes){
        MenuNode tarMenu = findDish(dishName);
        boolean check = true;
        int[] index = tarMenu.getDish().getStockID();
        for (int i = 0; i < index.length; i++) {
            for (int j = 0; j < stockVarSize; j++) {
                StockNode ptr = stockVar[j];
                while(ptr != null){
                    if (index[i] == ptr.getIngredient().getID()){
	...
	// WRITE YOUR CODE HERE
        ...
        return check; // update the return value
    }
    ...
    public void order (String dishName, int quantity){
        ...
        if (check == true){
            ...
            item.setType("order");
            item.setItem(dishName);
            item.setAmount(quantity);
            item.setSuccess(true);
            addTransactionNode(item);
            MenuNode dish = findDish(dishName);
            item.setProfit(dish.getDish().getProfit() * quantity);
            int index[] = dish.getDish().getStockID();
            for (int i = 0; i < index.length; i++) {
                updateStock(null, index[i], -quantity);
            }
        }
        else{
            MenuNode dish = findDish(dishName);
            ...
            MenuNode ptr = dish;
            MenuNode head = null;
            boolean rechecker = false;
            String cat = ptr.getDish().getCategory();
            for (int i = 0; i < categoryVar.length; i++) {
                if (cat.equalsIgnoreCase(categoryVar[i])){
                    head = menuVar[i];
                    break;
                }
            }
            while(ptr != null){
                ...
                if(checkDishAvailability(ptr.getDish().getName(), quantity) == true){
                    item.setType("order");
                    item.setItem(ptr.getDish().getName());
                    item.setAmount(quantity);
                    item.setSuccess(true);
                    addTransactionNode(item);
                    dish = findDish(ptr.getDish().getName());
                    item.setProfit(dish.getDish().getProfit() * quantity);
                    int index[] = dish.getDish().getStockID();
                    for (int i = 0; i < index.length; i++) {
                        updateStock(null, index[i], -quantity);
            ...
            if (rechecker == false && head != null){
                while(head != stop){
                    TransactionData item = new TransactionData();
                    if(checkDishAvailability(head.getDish().getName(), quantity) == true){
                        item.setType("order");
                        item.setItem(head.getDish().getName());
                        item.setAmount(quantity);
                        item.setSuccess(true);
                        addTransactionNode(item);
                        dish = findDish(head.getDish().getName());
                        item.setProfit(dish.getDish().getProfit() * quantity);
                        int index[] = dish.getDish().getStockID();
                        for (int i = 0; i < index.length; i++) {
                            updateStock(null, index[i], -quantity);
    ...
    public double profit () {
        ...
        double money = 0.0;
        while(ptr!=null){
            money = money + ptr.getData().getProfit();
            ptr = ptr.getNext();
        }
	// WRITE YOUR CODE HERE
        return money; // update the return value
    }
    ...
    public void restock (String ingredientName, int quantity){
        ...
        for (int i = 0; i < stockVarSize; i++) {
            ...
            if (ptr!=null){
                costs = ptr.getIngredient().getCost() * quantity;
                double money = profit();
                if(money >= costs ){
                    check = true;
                    break;
